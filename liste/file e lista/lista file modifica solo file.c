/* quello che fa questo programma è popolare la lista l con degli interi casuali.
...
nodo*elocc2(nodo*p,int a); ///sarebbe la funzione simile a k lista (metti k in un'altra lista). non viene eliminato nulla,però.
void stampa(nodo*p);
...
int main ()
{
    ...
    scanf("%d",&n);
    tmp=elocc2(l,n);  ///così facendo,sto lavorando solo su tmp senza modificare l. RICORDA: elocc2 non è la solita elimina occorrenza ma una funzione uguale a klista(metti k in un'altra lista)
