...
int main ()
{
    ...
    nodo*l=NULL;
    int i=0,n,dim;
                                        ///QUESTA "VERSIONE" Dà PER SCONTATO CHE IL FILE A.TXT CONTENGA NUMERI
    printf("\nleggo quello che c'è nel file: \n");
    l=leggi(pf);
    stampa(l);
                                                ///ricerco un elemento nel file
    /*printf("\nquale elemento vuoi ricercare nel file?  ");   ///ovviamente,il file a.txt,deve avere dei numeri
    scanf("%d",&n);
    l=leggi(pf);
    if(ricerca(l,n))
    {
   ...
   /* printf("\nquale occorrenza vuoi cercare?  "); ///ovviamente,il file a.txt,deve avere dei numeri
    scanf("%d",&n);
    l=leggi(pf);
...
nodo*scrivi(FILE*pf,int dim)
{
    ...
    if(pf)
    {
        for(i=0;i<dim;i++)
        {
            ...
            scanf("%d",&tmp->primo);
            fprintf(pf,"%d  ",tmp->primo);    ///FAI ATTENZIONE  a non scrivere '->'
            p=incoda(p,tmp->primo);
        }
    }
    else
